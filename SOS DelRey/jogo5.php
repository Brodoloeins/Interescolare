    ...
    <main id="jogos">
        <div class="center">
            <?php
                ...
                if(!$_POST["opt"]){
                    $sql = "SELECT * FROM `sos.delrey.jogos` WHERE categoria = '$categoria' AND numero = '5'";
                    ...
                    if(mysqli_num_rows($num) > 0){
                        while($linha = mysqli_fetch_assoc($num)){
                           echo "<button id=\"boneco\"><img id=\"img\" src=\"./img/personagem.png\" title=\"Para começar clique\"></button>";
                            echo "<form class=\"perg\" action=\"jogo5.php\" method=\"post\" >";
                    ...
                    $sql = "SELECT * FROM `sos.delrey.jogos` WHERE categoria = '$categoria' AND correta = '$opt' AND numero = '5'";
                    $num = mysqli_query($conexao, $sql);
                    if(mysqli_num_rows($num) > 0){
                        while($linha = mysqli_fetch_assoc($num)){
                            echo "<button id=\"boneco\"><img id=\"img\" src=\"./img/personagem.png\" title=\"Para começar clique\"></button>";
                        echo "<p id=\"correction\">Parabéns, você acertou essa questão <a href=\"./final.php\" style=\"font-size: 35px; float:right; background-color: white; color: red; text-decoration:none;\" > > </a></p>";
                            echo "<br>";
                            $_SESSION['acertos'] += 1;
                        }
                    }else{
                        ...
                        while($linha = mysqli_fetch_assoc($num)){
                            echo "<button id=\"boneco\"><img id=\"img\" src=\"./img/personagem.png\" title=\"Para começar clique\"></button>";
                            echo "<p id=\"correction\">Infelizmente você errou essa questão, a resposta correta é a opção ".$linha["correta"]." <a href=\"./final.php\" style=\"font-size: 35px; float:right; background-color: white; color: red; text-decoration:none;\" > > </a></p>";
                            echo "<br>";
                        }
                    }
                }
